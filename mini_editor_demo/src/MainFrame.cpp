...
int CMainFrame::OnCreate(LPCREATESTRUCT createStruct)
{
    ...
    m_status.Create(_T("  Starting QtKit mini editor..."),
                    WS_CHILD | WS_VISIBLE | SS_CENTERIMAGE,
                    CRect(0, 0, 0, 0), this, 0);
    m_status.SetFont(GetFont());

    m_controller.reset(new CMiniEditorController());
    m_viewController.reset(new CMiniEditorViewController(m_controller.get(), GetSafeHwnd()));
...
LRESULT CMainFrame::OnQmlClick(WPARAM wParam, LPARAM)
{
    ...
    {
        LayoutChildren();
        m_status.SetWindowText(_T("  Mini editor ready - import media to begin."));
    ...
    {
    case kClickAdd:
        text.Format(_T("  Timeline clip: %s"),
                    m_controller->HasTimelineClip() ? _T("added") : _T("import media first"));
        break;
    case kClickPlay:
    ...
    case kClickStop:
        ...
        break;
    case kClickSplit:
        text.Format(_T("  Split markers: %d"), m_controller->SplitCount());
        break;
    case kClickReset:
        text = _T("  Clip adjustments reset.");
        break;
    default:
        break;
    }
...
void CMainFrame::ImportMedia(const CString& path)
{
    ...
    SetWindowText(_T("Mini Editor - ") + name);
