...
void QtKitHost::Log(const char* fmt, ...)
{
    ...
    std::string strPath = std::string(szDir) + "MiniEditorDemo.log";
...
bool QtKitHost::Start(std::function<void()> onReady)
{
    ...
    m_pQtKit->setAppName("MiniEditorDemo");
    Log("setAppName ok");
    ...
    const std::string strUserIni = std::string(szTemp) + "MiniEditorDemo_UserConfig.ini";
    const std::string strDsaIni  = std::string(szTemp) + "MiniEditorDemo_DontShowAgain.ini";
