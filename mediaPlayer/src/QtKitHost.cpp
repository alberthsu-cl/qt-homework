...
void QtKitHost::Log(const char* fmt, ...)
{
    ...
    std::string strPath = std::string(szDir) + "MediaPlayer.log";
...
bool QtKitHost::Start(std::function<void()> onReady)
{
    ...
    m_pQtKit->setAppName("MediaPlayer");
    Log("setAppName ok");
    ...
    const std::string strUserIni = std::string(szTemp) + "MediaPlayer_UserConfig.ini";
    const std::string strDsaIni  = std::string(szTemp) + "MediaPlayer_DontShowAgain.ini";
