...
public:
    ...
    // Appends one line to %TEMP%\MediaPlayer.log. The demo has no console and
    ...
    // which is why the normal build stages a private copy under bin_x64\.
    // The post-build deployment additionally copies only MediaPlayer.exe
    // and its private QML folder beside PDR.exe for PDR-hosted diagnostics.
